...
function v($scope,$rootScope,myService,$http,$location,$mdDialog,Upload){
	...
	var posting = $http({
		...
		$scope.upload = function (file,imageNum) {
			
			$http({
			method:'POST',
            		url: '/addimage', //webAPI exposed to upload the file
...
function tempCon($scope,$rootScope,$http){
...
app.run(['$rootScope', '$location',
    function ($rootScope, $location) {
        ...
        $rootScope.$on('$locationChangeStart', function (event, next, current) {
            ...
            if ($location.path() == '/homePage' && !$rootScope.dataa) {
                $location.path('/');
            }
        });
    }]);
