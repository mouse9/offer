...
app.post('/addData.html',function(req,res){
	...
	con.query('insert into vendor (userId,password,number,emailAddress,offerTable,address,mainCategory,timing,closedOn,nameOfShop) values ( '+userId+password+number+emailAddress+offerTable+address+category+timings+closedOn+nameOfShop+');',function(err,rows){
		  if(err) throw err;
...
app.post('/shopsdata.html',function(req,res){
		...
		con.query('select userId,number,imageUrl1,imageUrl2,imageUrl3,address,timing,closedOn,nameOfShop from vendor where mainCategory =\''+category+'\';',function(err,rows){
		if(err) throw err;
